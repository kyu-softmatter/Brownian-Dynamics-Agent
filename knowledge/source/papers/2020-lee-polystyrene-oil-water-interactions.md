---
type: source
kind: paper
lab_authored: true
title: Interactions between polystyrene particles with diameters of several tens to hundreds of micrometers at the oil-water interface
authors:
  - "Lee HE"
  - "Choi KH"
  - "Xia M"
  - "Kang DW"
  - "Park BJ"
year: 2020
journal: J. Colloid Interface Sci. 560, 838-848
doi: 10.1016/j.jcis.2019.10.095
source_url: "https://doi.org/10.1016/j.jcis.2019.10.095"
access: paywall
engine: 실험
reproduced: no
parameters_extracted: no
affiliation: Kyung Hee University (Park BJ group) — Takatori 랩 이전
ingested_at: 2026-07-27
ingested_by: agent
tags:
  - "adjacent"
---
# Interactions between polystyrene particles with diameters of several tens to hundreds of micrometers at the oil-water interface

## 왜 이 위키에 있는가

수십~수백 µm 폴리스티렌 입자의 계면 상호작용. **크기 스케일링**.

**관련도:** `adjacent` · **소속:** Park BJ 그룹 (경희대) — Takatori 랩 합류 이전 연구

## 원문 접근

- DOI: https://doi.org/10.1016/j.jcis.2019.10.095
- ⚠️ **paywall.** 기관 접근권으로 받아 `knowledge/raw/lab/2020-lee-polystyrene-oil-water-interactions.pdf` 에 두면 증류 가능

## 추출 대기

> ⚠️ **수집 기록**이다. 추출 전까지 근거로 인용하지 않는다.

측정 기반 논문에서 뽑을 것:

| 항목 | 값 | 위치 |
|---|---|---|
| 계 (입자 재질·크기·매질) | | |
| 물성 (η, T, 이온세기, ζ-전위) | | |
| **측정된 쌍퍼텐셜 `U(r)` 형태·크기** | | |
| Debye 길이 `κ⁻¹` / `κσ` | | |
| 불확실도 | | |
| **BD 퍼텐셜과 대조 가능한 수치** | | |

→ 대조 가능한 수치가 있으면 `knowledge/wiki/benchmarks/`에 등재하고
`systems/charged-colloid--pair-potential` 카드에 연결한다.
