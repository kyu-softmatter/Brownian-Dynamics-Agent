...
| **L2 engine (`bdbot/`)** | ✅ 36 modules, L0→L7. All 8 cases on the `@RUN.builder` contract. The 5 pre-existing cases were re-run after migration and matched to 15 decimal places |
| **L2 pipeline half (`simbot/`)** | ✅ 19 modules, S2/S6/S7/S8 + sealing + `INCONCLUSIVE`. ⚠️ **one runner only** (`passive-sphere--harmonic-trap`) |
| **L3 knowledge** | 🔶 52 wiki pages · 45 distillations · 160 entries · 239 post-mortems — **but two unmerged schemas** |
| **L4 artifacts** | ✅ 306 specs · 283 run directories · 273 with `metrics.json` |
