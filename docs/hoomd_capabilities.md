...
| 16 | `md.force.Constant` 균일 중력 (BD) | ✓ `v = F/γ` 를 +0.13 % (0.43σ) 로 재현 | `passive-sphere--sedimentation` |
| 17 | `md.external.wall.Gaussian` + `wall.Plane` 바닥벽 | ✓ 주기경계 반대편에서 힘이 **정확히 0** (최소이미지 미적용) · 중력 하에서 500개 중 0개 통과 | 동일 |

**로우데이터 5계층(§9)이 전부 실동작 확인되었습니다.**

16·17 은 2026-09-16 추가분이다 (`verify/verify_sedimentation_wall.py`). 세 단계를
분리해서 측정했고, 비상호작용 밀도 프로파일이 barometric law 를 재현한다:
`l_g` 적합 **13.3731 ± 0.1125 d** vs 부과값 13.3750 d (**−0.014 %, 0.02σ**),
`dt` 를 5e-4 → 2e-4 로 바꿔도 1.575 % 이내. 두 개의 새 함정(21·22)이 이 과정에서
측정됐고 `bd-hoomd` 에 들어갔다.
