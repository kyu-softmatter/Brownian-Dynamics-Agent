...
MUTANTS = [
    ...
    dict(id="DIM-given-source-kind", file="bdbot/physical.py", kind="replace",
         anchor='    if basis == "given":\n        kind = e.get("source_kind")',
         into='    if False:\n        kind = e.get("source_kind")',
         gate="`basis: given` must say which input, and the file must exist",
         expect="caught",
         fast=["tests/test_dim_gate.py"]),
    ...
    SEAL_FILE,                 # the pre-registration""",
         ...
         expect="caught",
         fast=["tests/test_run_gates.py"]),
    # ── the seal, rewritten 2026-09-15 after it was measured to have hashed
    #    nothing on all 21 archived seal directories ───────────────────────
    dict(id="SEAL-hashes-something", file="simbot/io.py", kind="replace",
         anchor="    return SealVerdict(ok=bool(verified) and not (changed or missing or unsealed),",
         into="    return SealVerdict(ok=not (changed or missing or unsealed),  # MUTANT",
         gate="a seal that hashed zero documents cannot pass",
         expect="caught", fast=["tests/test_s8_io.py"]),
    dict(id="SEAL-content-compared", file="simbot/io.py", kind="replace",
         anchor="        (verified if sha256_file(here) == want else changed).append(name)",
         into="        verified.append(name)  # MUTANT",
         gate="the sealed document's content is actually hashed and compared",
         expect="caught", fast=["tests/test_s8_io.py"]),
    dict(id="SEAL-missing-not-skipped", file="simbot/io.py", kind="replace",
         anchor="        if not here.exists():\n            missing.append(name)\n            continue",
         into="        if not here.exists():\n            continue  # MUTANT",
         gate="a sealed document that is gone is reported, not skipped",
         expect="caught",
         why="this mutant IS the original defect in miniature: the pre-2026-09-15 "
    ...
    dict(id="SEAL-unsealed-detected", file="simbot/io.py", kind="replace",
         ...
         gate="a document written after sealing is reported unsealed",
         expect="caught", fast=["tests/test_s8_io.py"]),
    dict(id="SEAL-drift-reported", file="simbot/io.py", kind="replace",
         anchor="        if here.resolve() != recorded.resolve():\n            drifted.append(name)",
         into="        pass  # MUTANT",
         gate="a recorded path that no longer matches is reported as drift",
         expect="caught", fast=["tests/test_s8_io.py"]),
